...
Hud hud(int screen_w, int screen_h, int num_actions) {
    ...
    out.panel = {hud_x, hud_y, hud_width, hud_height};
    ...
    for (int i = 0; i < num_actions; ++i) {
        ...
        out.cards.push_back({ix, start_y, item_width, item_height});
    }
    return out;
}
...
Dialog dialog(int screen_w, int screen_h) {
    ...
    out.card = {x, y, card_w, card_h};

    int inner_x = x + pad;
    int inner_w = card_w - 2 * pad;
    int btn_w = (inner_w - btn_gap) / 2;
    int by = y + card_h - pad - btn_h;
    out.cancel = {inner_x, by, btn_w, btn_h};
    out.confirm = {inner_x + btn_w + btn_gap, by, btn_w, btn_h};
    return out;
}
